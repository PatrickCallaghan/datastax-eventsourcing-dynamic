...
//	private static AtomicLong totalCount = new AtomicLong(0);
	...
	private static String eventTable = keyspaceName + ".eventsource_dynamic";

	private static final SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyyMMdd");

	private static final String INSERT_INTO_EVENTS = "insert into " + eventTable + "(date, bucket, id, aggregatetype, host, loglevel, data, time, eventtype, p_) values (?,?,?,?,?,?,?,?,?,?)";
	private static final String SELECT_BY_DATE = "select * from " + eventTable + " where date =? and bucket = ? allow filtering"; 
	...
	public void insertEvent(Event event) {
		...
		bs.bind(date, bucket, event.getId(), event.getAggregateType(), event.getHost(), event.getLoglevel(), event.getData(),
				event.getTime(), event.getEventtype(), event.getProperties());
